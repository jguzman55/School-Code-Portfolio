...
;  ]
   ...
   ask wolves[
  if chase[
   if not any? sheep [ stop ];here
    face one-of sheep   ;; needs to face nearest sheep;not facing nearest; only chase when there is one left
    wiggle                        ;; randomly turn a bit
    forward 0.8                   ;; move forward
    ask sheep-here [die]        ;; any sheep here die
  ]
  ]
end
...
;    face one-of sheep    ;; face nearest sheep
;    wiggle                        ;; randomly turn a bit
